...
@pinned_only
def test_the_property_comes_from_the_pinned_open_not_from_luck(
    tmp_path: Path, monkeypatch
) -> None:
    """Same scenario with the pinned path disabled -- the write DOES get redirected.

    This is how the fix is verified, because the pre-fix tree cannot run this file at
    all (`_supports_pinned_walk` did not exist, so collection fails). Forcing the
    fallback reproduces the old behaviour exactly.

    It doubles as the honest statement of the platform gap: Windows has no `dir_fd`
    support in the stdlib, so it takes this path, and this test is what that costs. If
    someone later claims parity, this assertion is where the claim breaks.
    """
    monkeypatch.setattr(
    ...
    # An EXISTING file in the attacker's directory is no longer reachable at all:
    # exclusive creation refuses the name rather than truncating it. That is a second
    # guard arriving after this test was written, and it does not make pinning
    # redundant -- the redirect itself is still there, which is what the fresh name
    # below demonstrates.
    with pytest.raises(UnsafePathError):
        open_write_nofollow(out / "report.json", what="JSON report")
    assert victim.read_text(encoding="utf-8") == "PRECIOUS\n"

    fd = open_write_nofollow(out / "fresh.json", what="JSON report")
    with os.fdopen(fd, "w", encoding="utf-8") as fh:
        fh.write("REDIRECTED")

    assert (victim_dir / "fresh.json").read_text(encoding="utf-8") == "REDIRECTED", (
        "the unpinned path no longer follows a swapped parent -- if this now holds, "
        "the fallback gained ancestor protection and this test's premise is stale"
    )

...
way it is. These are **arguments, not contracts**: nothing here is normative, and
no code change is required to keep a post true.
...
is a position — it can be argued with, and it can age.

## Index

| Post | What it argues |
|---|---|
| [vibe-coding-generations.md](vibe-coding-generations.md) | The 1.0-to-4.0 ladder measures the wrong variable. The real gate is blast radius, 3.0 is oracle engineering rather than a better session list, and accountability ports into an agent system as dynamic permission rather than as a conscience. |

## Writing one

- File as `<topic>.md`, kebab-case, English.
- Open with front matter — `title`, `author`, `created` — then an H1, then the
  argument. No `Last Updated:` line: git records history.
...
- Say which parts are unbuilt. A post that reads as a roadmap commitment belongs
  in `request-for-change/` instead.

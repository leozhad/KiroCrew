...
def _mcp_apps_enabled() -> bool:
    """Feature gate for MCP Apps — the env kill switch, and nothing else.

    Capability follows ROUTING, not a preference. This function only ever runs
    inside a backend, and a backend only exists because a stub reached the broker
    for a server the operator stubbed. By the time control is
    here the opt-in has already happened, so there is no second switch to
    consult: a preference could not grant the feature (with no stub there is no
    render or callback path to grant) and could not honestly withdraw it either.

    ``KIROCREW_MCP_APPS`` stays as an absolute kill switch — for tests, the e2e
    harness, and an operator who wants a stubbed server's backend shared without
    its server-authored UI. Any value that reads as off disables rendering;
    anything else, absent included, leaves it on.

    ``mcp_gateway.apps_enabled`` is deliberately NOT read: it is deprecated and
    ignored, so a released config still carrying the key changes behaviour in
    neither direction. No config is loaded here at all, which also removes the
    fail-closed-on-unreadable-config branch this gate used to need.
    """
    return mcp_apps_env_override() is not False

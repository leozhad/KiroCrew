...
MCP Apps has no switch of its own. Capability follows THE STUB: the stub a stubbed
server gets is what carries the render and callback path, so routing the server is
what grants the feature. `_mcp_apps_enabled()` therefore reads only the env kill
switch:
...
| `KIROCREW_MCP_APPS` unset | enabled — reaching the gate already means the server was stubbed |
